...
class LaserScanWaymo:
    ...
    def __init__(self, project=False, H=64, W=2650, fov_up=2.25, fov_down=-18.1, 
    ...
    def open_scan(self, filename):
        ...
        # if all goes well, open pointcloud
        scan = np.load(filename)['array'].astype(np.float32)
        scan = np.transpose(scan, (1, 2, 0))
        scan = np.reshape(scan, (64 * 2650, 5))
        self.imglblmask = scan[:, 0] != -1
        scan = scan[self.imglblmask][...,1:]
        rem01 = True
        if rem01:
            scan[...,-1] = np.clip(scan[...,-1],0,1)
...
class SemLaserScanWaymo(LaserScanWaymo):
    """Class that contains LaserScan with x,y,z,r,sem_label,sem_color_label,inst_label,inst_color_label"""
    EXTENSIONS_LABEL = ['.npz']

    def __init__(self, sem_color_dict=None, project=False, H=64, W=2650, fov_up=2.25, fov_down=-18.1, max_classes=24,DA=False,flip_sign=False,drop_points=False):
        super(SemLaserScanWaymo, self).__init__(project, H, W, fov_up, fov_down,DA=DA,flip_sign=flip_sign,drop_points=drop_points)
        self.reset()
    ...
    def reset(self):
        """ Reset scan members. """
        super(SemLaserScanWaymo, self).reset()
    ...
    def open_label(self, filename):
        ...
        # if all goes well, open label
        label = np.load(filename)['array'].astype(np.int32)
        label = label.reshape((-1))
        label = label[self.imglblmask]
